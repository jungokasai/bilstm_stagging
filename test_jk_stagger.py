...
def train_stagger(config):
    ...
    train_data_info = ' --text_train {} --jk_train {} --tag_train {}'.format(os.path.join(base_dir, 'sents', 'train.txt'), os.path.join(base_dir, 'predicted_pos', 'train.txt'), os.path.join(base_dir, 'gold_stag', 'train.txt'))
    dev_data_info = ' --text_test {} --jk_test {} --tag_test {}'.format(os.path.join(base_dir, 'sents', 'dev.txt'), os.path.join(base_dir, 'predicted_pos', 'dev.txt'), os.path.join(base_dir, 'gold_stag', 'dev.txt'))
...
#    best_model = get_best_model(config_file)
    best_model = '../project/tag_wsj/Stagging_Model_LM/1-2-512-0-5-100-100-10-0.01-0.5-0.5-0.8-Super_models/best_model'
