...
class Stagging_Model_Concat_Adv(object):
    ...
    def add_word_embedding(self): 
        with tf.device('/cpu:0'):
            ...
            embedding = self.normalize_embedding(embedding, self.loader.word_freqs)
    ...
    def add_suffix_embedding(self):
        with tf.device('/cpu:0'):
            ...
            embedding = self.normalize_embedding(embedding, self.loader.suffix_freqs)
    ...
    def add_char_embedding(self):
        with tf.device('/cpu:0'):
            ...
            embedding = self.normalize_embedding(embedding, self.loader.char_freqs)
    ...
    def add_jackknife_embedding(self):
        with tf.device('/cpu:0'):
            ...
            embedding = self.normalize_embedding(embedding, self.loader.jk_freqs)
    ...
    def add_lstm(self, inputs, i, name, backward=False, adv=False):
        ...
        weights = get_lstm_weights('{}_LSTM_layer{}'.format(name, i), inputs_dim, self.opts.units, tf.shape(inputs)[1], self.hidden_prob, reuse=adv)
        if backward:
    ...
    def normalize_embedding(self, embedding, freqs):
        ...
        ## 
        e_embedding = tf.reduce_sum(embedding*tf.expand_dims(freqs, 1), 0, keep_dims=True)
        v_embedding = tf.reduce_sum((embedding-e_embedding)**2*tf.expand_dims(freqs, 1), 0, keep_dims=True)
    ...
    def add_adversarial_loss(self, loss):
        ...
        for i, embedding in enumerate(self.adv_embedding):
            normalized_gradient = tf.stop_gradient(gradients[i]/tf.norm(gradients[i], axis=-1, keep_dims=True))## do not take second-order gradient
            epsilon = 0.001*tf.sqrt(tf.cast(tf.shape(embedding)[-1], tf.float32))
            ...
            if i == len(self.adv_embedding) - 1: ## char computation
                if self.opts.chars_dim > 0:
                    new_embedding = self.add_dropout(new_embedding, self.input_keep_prob)
                    new_embedding = encode_char(new_embedding, self.char_weights) ## [seq_len, batch_size, nb_filters]
            new_embeddings.append(new_embedding)
        inputs_tensor = tf.concat(new_embeddings, 2) ## [seq_len, batch_size, inputs_dim]
        adv_loss, projected_outputs = self.feed_network_inputs(inputs_tensor, adv=True)
    ...
    def feed_network_inputs(self, inputs_tensor, adv=False):
        ...
        for i in xrange(self.opts.num_layers):
            forward_outputs_tensor = self.add_dropout(self.add_lstm(inputs_tensor, i, 'Forward', adv=adv), self.keep_prob) ## [seq_len, batch_size, units]
            if self.opts.bi:
                backward_outputs_tensor = self.add_dropout(self.add_lstm(tf.reverse(inputs_tensor, [0]), i, 'Backward', True, adv=adv), self.keep_prob) ## [seq_len, batch_size, units]
        ...
        projected_outputs = tf.map_fn(lambda x: self.add_projection(x, reuse=adv), lstm_outputs) #[seq_len, batch_size, nb_tags]
    ...
    def __init__(self, opts, test_opts=None):
        ...
        adv_loss, _ = self.add_adversarial_loss(clean_loss) ## do not care about adversarial prediction accuracy
        alpha = 0.5
        #alpha = 1.0
