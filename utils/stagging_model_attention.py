...
class Stagging_Model_Attention(Stagging_Model):
    ...
    def add_attention(self, lstm_outputs, cells):
        ## lstm_outputs: [n, b, d]
        shape = lstm_outputs
        weights = get_attention_weights('attention', self.outputs_dim)
        outputs = attention_equation(lstm_outputs, cells, weights)
        return outputs
