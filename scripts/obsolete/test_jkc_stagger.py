...
def train_stagger(config):
    ...
    train_data_info = ' --text_train {} --jk_train {} --tag_train {}'.format(os.path.join(base_dir, 'sents', 'train.txt'), os.path.join(base_dir, 'predicted_cpos', 'train.txt'), os.path.join(base_dir, 'gold_stag', 'train.txt'))
    dev_data_info = ' --text_test {} --jk_test {} --tag_test {}'.format(os.path.join(base_dir, 'sents', 'dev.txt'), os.path.join(base_dir, 'predicted_cpos', 'dev.txt'), os.path.join(base_dir, 'gold_stag', 'dev.txt'))
...
def test_stagger(config, best_model, data_types):
    ...
    for data_type in data_types:
        output_file = os.path.join(base_dir, 'predicted_stag_0.5', '{}.txt'.format(data_type))
        inputs = {}
        inputs[10] = output_file
        if not os.path.isdir(os.path.dirname(output_file)):
            os.makedirs(os.path.dirname(output_file))
        output_info = ' --save_tags {} --get_accuracy'.format(output_file)
        test_data_info = ' --text_test {} --jk_test {} --tag_test {}'.format(os.path.join(base_dir, 'sents', '{}.txt'.format(data_type)), os.path.join(base_dir, 'predicted_cpos', '{}.txt'.format(data_type)), os.path.join(base_dir, 'gold_stag', '{}.txt'.format(data_type)))
