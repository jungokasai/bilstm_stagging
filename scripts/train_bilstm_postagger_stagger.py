...
from tools.jackknife import split4jk, combine4jk
...
def train_test_jk_postagger(config):
    split4jk(config)
    base_dir = config['data']['base_dir']
    base_command = 'python bilstm_stagger_main.py train --task POS_models --base_dir {}'.format(base_dir)
    for ith in xrange(config[nb_folds]):
        print('jackknifing number {}'.format(ith+1))
        train_data_info = ' --text_train {} --jk_train {} --tag_train {}'.format(os.path.join(base_dir, 'sents', 'train{}.txt'.format(ith)), os.path.join(base_dir, 'gold_pos', 'train{}.txt'.format(ith)), os.path.join(base_dir, 'gold_pos', 'train{}.txt'.format(ith)))
        dev_data_info = ' --text_test {} --jk_test {} --tag_test {}'.format(os.path.join(base_dir, 'sents', 'dev{}.txt'.format(ith)), os.path.join(base_dir, 'gold_pos', 'dev{}.txt'.format(ith)), os.path.join(base_dir, 'gold_pos', 'dev{}.txt'.format(ith)))
        model_config_dict = config['pos_parameters']
        model_config_info = ''
        for option, value in model_config_dict.items():
            model_config_info += ' --{} {}'.format(option, value)
        complete_command = base_command + train_data_info + dev_data_info + model_config_info
        subprocess.check_call(complete_command, shell=True)
        best_model = get_best_model(config)
        data_types = ['dev{}.txt'.format(ith)]
...
#    print('Run Jackknife Training of POS tagging for Supertagging')
    print('Jacknife Train POS tagger')
    train_test_jk_postagger(config_file)
    print('Train POS tagger')
    train_postagger(config_file)
